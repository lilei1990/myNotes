    ...
    if (fuse) {
        ...
        if (results.length !== 0) {
            ...
            for (let item in results) {
                resultSet += `<li class="post-entry"><header class="entry-header">${decodeURIComponent(results[item].item.permalink.split("/").slice(-2)[0])}&nbsp;»</header>` +
                    `<a href="${results[item].item.permalink}" aria-label="${results[item].item.title}"></a></li>`
                     // console.log(Object.entries( results[item].item));
                     //   console.log(decodeURIComponent(results[item].item.permalink));
